...
def linear_assignment(X):
    ...
    #indices = np.array(indices, dtype=np.int8)
...
class _HungarianState:
    ...
    def __init__(self, cost_matrix):
        ...
        self.Z0_r = 0
        self.Z0_c = 0
        #self.path = np.zeros((n + m, 2), dtype=np.int8)
        #self.marked = np.zeros((n, m), dtype=np.int8)
...
def _step4(state):
    ...
    #C = (state.C == 0).astype(np.int8)
    C = (state.C == 0).astype(np.int)
    covered_C = C * state.row_uncovered[:, np.newaxis]
    #covered_C *= state.col_uncovered.astype(dtype=np.int8, copy=False)
    ...
    while True:
        ...
        if covered_C[row, col] == 0:
            ...
            if not state.marked[row, star_col] == 1:
                ...
                covered_C[:, col] = C[:, col] * (
                    #state.row_uncovered.astype(dtype=np.int8, copy=False))
